      ...
      {step === 'result' && recommendation && (
        <div className="mt-4">
          ...
          <div className="mt-2 rounded-xl bg-accent/15 p-4">
            ...
            <Link
              to={`/programs/${recommendation.programId}?quizReason=${encodeURIComponent(recommendation.briefReason)}`}
              className="mt-3 block rounded-lg bg-accent py-2 text-center text-sm font-medium text-bg"
            >
              View program
            </Link>
          </div>
              ...
              <ul className="mt-2 space-y-2">
                {recommendation.alternatives.map((alt) => (
                  <li key={alt.programId}>
                    <Link
                      to={`/programs/${alt.programId}?quizReason=${encodeURIComponent(alt.reason)}`}
                      className="block rounded-xl bg-bg-surface p-3 hover:bg-bg-raised"
                    >
